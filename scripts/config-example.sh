#!/bin/bash

# Template de configuração do media-processor
# Copie este arquivo para config.sh e preencha com suas credenciais

# Token do bot do Telegram (obtenha com @BotFather)
export TELEGRAM_BOT_TOKEN="SEU_BOT_TOKEN_AQUI"

# ID do chat do Telegram (obtenha enviando mensagem para seu bot e verificando em https://api.telegram.org/bot<TOKEN>/getUpdates)
export TELEGRAM_CHAT_ID="SEU_CHAT_ID_AQUI"

# TMDB
export TMDB_API_KEY="SUA_API_TMDB"
export TMDB_BASE_URL="https://api.themoviedb.org/3"
export TMDB_POSTER_URL="https://image.tmdb.org/t/p/w500"

# qBittorrent
export QB_URL="http://localhost:8080"
export QB_USER="admin"
export QB_PASS="sua_senha"
export QB_SAVE_PATH="/mnt/media/qbittorrent/downloads/"

# Jackett
export JACKETT_URL="http://localhost:9117"
export JACKETT_API_KEY="sua_chave_jackett"

# Jellyfin
export JELLYFIN_MEDIA_DIR="/mnt/media/jellyfin/media/"

# Para usar:
# 1. Copie este arquivo: cp config-example.sh config.sh
# 2. Edite com suas credenciais: nano config.sh
# 3. Carregue as variáveis: source config.sh
# 4. Execute o notificador normalmente

echo "Configuração carregada."
